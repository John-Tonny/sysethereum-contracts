...
import "../interfaces/VircleERC20I.sol";
import "@openzeppelin/upgrades/contracts/Initializable.sol";

contract VircleERC20Manager is Initializable {
    ...
    // Contract to trust for tx included in a vircle block verification.
    // Only vircle txs relayed from trustedRelayerContract will be accepted.
    address public trustedRelayerContract;


    mapping(uint32 => uint256) public assetBalances;
    // Vircle transactions that were already processed by processTransaction()
    mapping(uint => bool) private vircleTxHashesAlreadyProcessed;
    ...
    function contains(uint value) private view returns (bool) {
        return vircleTxHashesAlreadyProcessed[value];
    }

    function insert(uint value) private returns (bool) {
        if (contains(value))
            return false; // already there
        vircleTxHashesAlreadyProcessed[value] = true;
        return true;
    }
    ...
    modifier minimumValue(address erc20ContractAddress, uint value) {
        uint256 decimals = VircleERC20I(erc20ContractAddress).decimals();
        require(
    ...
    function wasVircleTxProcessed(uint txHash) public view returns (bool) {
    ...
    function processTransaction(
        ...
        // Add tx to the vircleTxHashesAlreadyProcessed and Check tx was not already processed
    ...
    function cancelTransferRequest(uint32 bridgeTransferId) public payable {
        ...
        require(bridgeTransfer.status == BridgeTransferStatus.Ok,
            "#VircleERC20Manager cancelTransferRequest(): Status of bridge transfer must be Ok");
        ...
        require(msg.sender == bridgeTransfer.tokenFreezerAddress, "#VircleERC20Manager cancelTransferRequest(): Only msg.sender is allowed to cancel");
        // if freezeBurnERC20 was called less than 1.5 weeks ago then return error
        // 0.5 week buffer since only 1 week of blocks are allowed to pass before cannot mint on vircle
        require((block.timestamp - bridgeTransfer.timestamp) > (net == Network.MAINNET? CANCEL_MINT_TIMEOUT: 36000), "#VircleERC20Manager cancelTransferRequest(): Transfer must be at least 1.5 week old");
        // ensure min deposit paid
        require(msg.value >= MIN_CANCEL_DEPOSIT,
            "#VircleERC20Manager cancelTransferRequest(): Cancel deposit incorrect");
    ...
    function cancelTransferSuccess(uint32 bridgeTransferId) public {
        ...
        require(bridgeTransfer.status == BridgeTransferStatus.CancelRequested,
            "#VircleERC20Manager cancelTransferSuccess(): Status must be CancelRequested");
        // check if timeout period passed (atleast 1 hour of blocks have to have passed)
        require((block.timestamp - bridgeTransfer.timestamp) > CANCEL_TRANSFER_TIMEOUT, "#VircleERC20Manager cancelTransferSuccess(): 1 hour timeout is required");
        ...
        VircleERC20I erc20 = VircleERC20I(bridgeTransfer.erc20ContractAddress);
        ...
        onlyTrustedRelayer
    {
        ...
        require(bridgeTransfer.status == BridgeTransferStatus.CancelRequested,
            "#VircleERC20Manager cancelTransferSuccess(): Status must be CancelRequested to Fail the transfer");
